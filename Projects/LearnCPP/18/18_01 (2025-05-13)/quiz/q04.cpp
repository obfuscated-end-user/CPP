...
//	constexpr int length { sizeof(array) / sizeof(array[0]) };		// use instead if not C++17 capable
	...
	for (int iteration { 0 }; iteration < length - 1; ++iteration) {
		// Accoutn for the fact that the last element is already sorted with each subsequent iteration
		// so our array "ends" one element sooner
		int endOfArrayIndex { length - iteration };

		bool swapped { false }; // Keep track of wether any elements were swapped this iteration
		...
		if (!swapped) {
			// iteration 0-based, but counting iterations is 1-based. So add 1 here to adjust:
