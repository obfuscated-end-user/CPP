...
void calculateHeight(double time, double height) {
    ...
    constexpr double g = 9.8;               // g constant
    calculatedHeight = (g * (time * time)) / 2;   // can't use exponents yet 
    std::cout << "At " << time << " seconds, the ball is at height: " << height - calculatedHeight << "meters\n";
...
int main() {
    ...
    calculateHeight(0, height);
    calculateHeight(1, height);
    calculateHeight(2, height);
    calculateHeight(3, height);
    calculateHeight(4, height);
    std::cout << "At 5 seconds, the ball is on the ground.\n";
    // calculateHeight(5, height); // lolwut
    return 0;
}
/*
solution
